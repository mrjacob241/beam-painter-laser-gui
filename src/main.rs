...
fn main() -> Result<(), eframe::Error> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_title("BeamPainter Laser GUI v0.5")
...
impl RustLaserApp {
    ...
    fn send_loaded_gcode(&mut self) {
        if self.is_printing() || self.loop_active {
            self.status = "Cannot start a new print while motion is active.".to_string();
            self.append_telemetry_log("PRINT", "Send blocked because motion is active");
            return;
        }
        if self.controller_reset_required {
            self.status = "Controller reset is required before starting a new print.".to_string();
            self.append_telemetry_log("PRINT", "Send blocked because controller reset is required");
            return;
        }
        if !self.serial_connected {
            self.status = "Connect to the controller before starting a print.".to_string();
            self.append_telemetry_log("PRINT", "Send blocked because serial is disconnected");
            return;
        }
        if self.gcode_lines.is_empty() {
            self.status = "No G-code loaded.".to_string();
            self.append_telemetry_log("PRINT", "Send blocked because no G-code is loaded");
            return;
        }
        self.append_telemetry_log(
            "ACTION",
        ...
        if let Err(err) = self.serial_worker.send(SerialCommand::SendLoadedGcode {
    ...
    fn toggle_loop_mode(&mut self) {
        ...
        if self.loop_active {
            ...
            self.loop_rectangle = None;
            self.status =
                "Loop stop requested. Controller reset will be sent automatically.".to_string();
    ...
    fn show_rx_fallback_prompt(&mut self) {
        ...
        self.append_telemetry_log("PRINT", "RX fallback continue prompt shown");
    }
    ...
    fn reject_rx_fallback_continue(&mut self) {
        ...
        self.status =
            "Stopping print and resetting controller session for a clean next start.".to_string();
        self.controller_reset_required = false;
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        while let Some(event) = self.serial_worker.try_recv() {
            match event {
                ...
                SerialEvent::Connected(state) => {
                    let was_printing = self.print_active;
                    ...
                    if !state {
                        ...
                        if was_printing {
                            ...
                            self.controller_reset_required = true;
                            self.status =
                                "Serial connection lost during print. Reset or reconnect before starting a new print."
                                    .to_string();
                        }
                    }
                ...
                SerialEvent::PrintStarted => {
                    ...
                    self.rx_buffer_used = 0;
                    self.status =
                        "Print in progress. Streaming G-code to the controller.".to_string();
                ...
                SerialEvent::PrintStalledRecoverable {
                    sent,
                    total,
                    in_flight,
                } => {
                    ...
                    self.controller_reset_required = true;
                    self.status = format!(
                        "RX tracking stalled after {sent}/{total} lines with {in_flight} acknowledgement(s) pending. Use Continue to track motion or Flush / Reset before a new print."
                    );
                    self.append_telemetry_log(
                        "PRINT",
                        &format!(
                            "Recoverable RX stall: sent={sent}, total={total}, in_flight={in_flight}"
                        ),
                    );
                }
                SerialEvent::LaserPosition { position } => {
                    ...
                    self.append_telemetry_log(
                        "POSITION",
                        &format!(
                            "Laser position updated: X{:.3} Y{:.3}",
                ...
                ui.menu_button("Settings", |ui| {
                    ...
                    ui.menu_button("gcode serial count", |ui| {
                            ...
                            .changed()
                        {
                            self.append_telemetry_log(
                                "SETTINGS",
                                "G-code serial mode set to char count",
                            );
                        }
                        if ui
                            .selectable_value(
                            ...
                            .changed()
                        {
                            self.append_telemetry_log(
                                "SETTINGS",
                                "G-code serial mode set to RX logs",
                            );
                        }
                    });
                    ui.label(format!(
                ...
                if self.is_debug_mode {
                    ui.menu_button("Debug", |ui| {
                        ...
                        let telemetry_changed = ui
                            .checkbox(&mut self.telemetry_enabled, "Rust_Laser_GUI_Telemetry")
                            .changed();
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if self.show_millimeter_grid {
                ...
                let fine_grid_alpha = (2.0 + zoom_boost * 2.5).round().clamp(2.0, 18.0) as u8;
