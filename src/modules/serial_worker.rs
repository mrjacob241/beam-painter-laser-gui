...
fn send_loaded_gcode_worker_rx(
    ...
    let mut fallback_ack_wait_announced = false;
    ...
    loop {
            ...
            if rx_free_hint.is_some() {
                ...
                if session_config.no_rx_prompt_exit {
                    if in_flight.is_empty() {
                        let _ = tx_evt.send(SerialEvent::Log(
                            "STATUS> 100% send reached without RX buffer status fields; `No RX Prompt Exit` is enabled, all local acknowledgements drained, concluding print."
                                .to_string(),
                        ));
                        break;
                    }
                    if !fallback_ack_wait_announced {
                        let _ = tx_evt.send(SerialEvent::Log(format!(
                            "STATUS> 100% send reached without RX buffer status fields; waiting for {} local acknowledgement(s) before concluding print.",
                            in_flight.len()
                        )));
                        fallback_ack_wait_announced = true;
                    }
                } else if !fallback_prompt_pending {
            ...
            > Duration::from_secs(STREAM_STALL_TIMEOUT_SECS)
        {
            ...
            let _ = tx_evt.send(SerialEvent::PrintStalledRecoverable {
                sent: next_index,
                total,
                ...
                for raw_line in reply.lines() {
                    ...
                    if text.starts_with('<') {
                        if let Some(report) = parse_grbl_status_report(text) {
                            ...
                            if let Some(reported_rx_used) = report.rx_used {
                                rx_used = reported_rx_used.min(GRBL_RX_LIMIT);
                                rx_free_hint = Some(GRBL_RX_LIMIT.saturating_sub(rx_used));
                                let _ = tx_evt.send(SerialEvent::RxBufferFill {
                                    used: rx_used,
                                    capacity: GRBL_RX_LIMIT,
                                });
...
fn loop_rectangle_worker(
    ...
    let loop_lines = [
        format!(
            "G1 X{:.3} Y{:.3} F{:.0}",
            vertices[1][0], vertices[1][1], loop_feed
        ),
        format!(
            "G1 X{:.3} Y{:.3} F{:.0}",
            vertices[2][0], vertices[2][1], loop_feed
        ),
        format!(
            "G1 X{:.3} Y{:.3} F{:.0}",
            vertices[3][0], vertices[3][1], loop_feed
        ),
        format!(
            "G1 X{:.3} Y{:.3} F{:.0}",
            vertices[0][0], vertices[0][1], loop_feed
        ),
    ];
    ...
    'looping: loop {
        ...
        while status_tick_rx.try_recv().is_ok() {
            if let Err(err) = bridge.send_status_query() {
                let _ = tx_evt.send(SerialEvent::Log(format!("Loop status query failed: {err}")));
                break 'looping;
            }
        }
            ...
            Ok(Some(reply)) => {
                for raw_line in reply.lines() {
                    ...
                    if lower == "ok" || lower.starts_with("error:") {
                        ...
                        if lower.starts_with("error:") {
                            let _ =
                                tx_evt.send(SerialEvent::Log(format!("Loop device error: {text}")));
                            break 'looping;
                        }
                        continue;
                    }
...
fn stop_loop_stream(
    ...
    match bridge.send_soft_reset() {
        ...
        Err(err) => {
            let _ = tx_evt.send(SerialEvent::Log(format!("Loop reset failed: {err}")));
        }
    }
    let _ = tx_evt.send(SerialEvent::Log(
...
fn stop_stream(
    ...
    if stop_reset_requested.load(Ordering::SeqCst) {
        match bridge.send_soft_reset() {
            ...
            Err(err) => {
                let _ = tx_evt.send(SerialEvent::Log(format!("Reset failed after stop: {err}")));
            }
        }
    }
...
fn parse_grbl_status_report(line: &str) -> Option<GrblStatusReport> {
    ...
    for (idx, field) in body.split('|').enumerate() {
        ...
        if let Some(rest) = strip_status_field(field, "Bf") {
        ...
        if let Some(rest) = strip_status_field(field, "Buf") {
            report.planner_free = parse_status_usize(rest);
            continue;
        }

        if let Some(rest) = strip_status_field(field, "RX") {
            report.rx_used = parse_status_usize(rest);
            continue;
        }

        if let Some(rest) = strip_status_field(field, "WPos") {
            report.position = parse_status_position(rest);
            continue;
        }

        if report.position.is_none() {
            if let Some(rest) = strip_status_field(field, "MPos") {
                report.position = parse_status_position(rest);
            }
        }
    }
...
fn strip_status_field<'a>(field: &'a str, expected_key: &str) -> Option<&'a str> {
